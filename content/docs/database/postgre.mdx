...
pg_dump -Fc nom_db > backup.dump  # Format custom (compressé)
...
<Tabs items={['Performance', 'Sécurité', 'Développement']}>
...
1. **Indexes**
   ...
   - Créez des indexes sur les colonnes utilisées dans WHERE, JOIN, ORDER BY
   - Utilisez `EXPLAIN ANALYZE` pour analyser vos requêtes
   - Évitez les indexes inutiles (coût de maintenance)
...
3. **Configuration**
   - Ajustez `shared_buffers` (généralement 25% de la RAM)
...
1. **Authentification**
   ...
   - Préférez l'authentification par certificat en production
...
3. **Chiffrement**
   ...
   - Utilisez `pgcrypto` pour chiffrer les données sensibles
...
3. **Types personnalisés**
   ...
   ```sql
   -- Créer un type ENUM
   CREATE TYPE statut_commande AS ENUM ('en_attente', 'traitement', 'expédié', 'livré');
