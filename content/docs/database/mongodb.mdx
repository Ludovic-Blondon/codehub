...
| ------------------------------------- | ------------------------------------ |
| `mongosh`                             | Démarrer le shell MongoDB            |
| `mongosh "mongodb://localhost:27017"` | Se connecter avec une URI            |
| `show dbs` ou `show databases`        | Lister toutes les bases de données   |
| `use nom_db`                          | Basculer vers une base de données    |
| `db`                                  | Afficher la base de données courante |
...
| `show roles`      | Lister les rôles disponibles     |
...
db.users.updateOne({ email: "alice@example.com" }, { $addToSet: { tags: "newsletter" } });
